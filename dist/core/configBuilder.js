...
export async function buildWebConfig(args, pm) {
    ...
    return {
        ...
        ui: args.ui ?? 'none',
        stateManager: args.stateManager ?? 'none',
        testing: 'none',
...
export async function buildFlutterConfig(args) {
    const flags = args.flags;
    const features = {
        firebase: flags['firebase'] ?? false,
        supabase: flags['supabase'] ?? false,
    ...
    return {
        ...
        stateManager: args.stateManager ?? 'riverpod',
        features,
        devops: { docker: features.docker, ci: flags['ci'] ? 'github' : 'none' },
    };
}
export async function buildProjectConfig(args) {
