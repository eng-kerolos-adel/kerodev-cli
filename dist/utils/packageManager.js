...
export async function detectPackageManagers() {
    return Promise.all(['bun', 'pnpm', 'yarn', 'npm'].map(checkPM));
}
export async function getPackageManagerInfo(pm) {
    return checkPM(pm);
}
export async function getDefaultPackageManager() {
    const available = await detectPackageManagers();
    for (const pm of ['bun', 'pnpm', 'yarn', 'npm']) {
        if (available.find(p => p.name === pm)?.available)
            return pm;
    }
    return 'npm';
}
 ...
 */
export function getNextCreateCommand(pm, projectName, opts) {
    const dlx = getDlx(pm);
    const flags = [
...
export function getCreateCommand(pm, framework, projectName, opts) {
    const dlx = getDlx(pm);
...
export function getAddCommand(pm, dev, ...packages) {
