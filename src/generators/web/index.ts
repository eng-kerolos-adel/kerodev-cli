...
import type { WebProjectConfig, GeneratorContext } from '../../types/index.js';
...
export async function generateWebProject(config: WebProjectConfig): Promise<void> {
  ...
  s1.start();
  try {
    const createCmd = getCreateCommand(
...
async function writeConfigs(ctx: GeneratorContext): Promise<void> {
...
async function enhancePackageJson(ctx: GeneratorContext): Promise<void> {
  ...
  const pkg = await fse.readJSON(pkgPath) as { scripts?: Record<string, string> };
...
async function installUiLibrary(ctx: GeneratorContext): Promise<void> {
  const cfg = ctx.config as WebProjectConfig;
  ...
  if (!pkgs) { s.warn(`${cfg.ui} install skipped`); return; }
  try {
...
async function setupDevOps(ctx: GeneratorContext): Promise<void> {
  const cfg = ctx.config as WebProjectConfig;
  const jobs: Promise<void>[] = [];
