...
export function generateEnvFiles(config: WebProjectConfig): [string, string] {
  ...
  if (config.features.firebase) {
    lines.push(
      ...
      `${prefix}FIREBASE_AUTH_DOMAIN=""`,
      `${prefix}FIREBASE_PROJECT_ID=""`,
      `${prefix}FIREBASE_STORAGE_BUCKET=""`,
      `${prefix}FIREBASE_MESSAGING_SENDER_ID=""`,
      `${prefix}FIREBASE_APP_ID=""`,
      '',
    );
  }
