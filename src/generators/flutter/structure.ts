...
import type { GeneratorContext, FlutterProjectConfig } from '../../types/index.js';
...
const ARCH_DIRS: Record<FlutterProjectConfig['architecture'], string[]> = {
  clean: [
    'lib/features',
    'lib/domain/entities',
...
export async function generateFlutterStructure(ctx: GeneratorContext): Promise<void> {
  ...
  await Promise.all(
    allDirs.map(d =>
      writeFile(path.join(base, d, '.gitkeep'), '')
    )
  );
...
async function generateExampleFeature(ctx: GeneratorContext): Promise<void> {
  const cfg = ctx.config as FlutterProjectConfig;
