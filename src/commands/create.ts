...
import type { WebProjectConfig, FlutterProjectConfig } from '../types/index.js';
...
export function registerCreateCommand(program: Command): void {
  program
    ...
    .allowUnknownOption()
    .action(async (projectName: string | undefined, ecosystem: string | undefined, extraArgs: string[], opts) => {
      brand.header();
      try {
        let config;
        ...
        if (!projectName) {
          ...
          if (opts.pm) parsed.packageManager = opts.pm;
          config = await buildProjectConfig(parsed);
        }

        // ── Validate ──────────────────────────────────────────────────────
        log.section('Validating configuration');
        const combined = mergeValidations(
          validateProjectName(config.name),
          await validateEnvironment(config),
          config.ecosystem === 'web' ? validateWebConfig(config as WebProjectConfig) : { valid: true, errors: [], warnings: [] }
        );
        ...
        if (config.ecosystem === 'web') {
          await generateWebProject(config as WebProjectConfig);
        } else {
          await generateFlutterProject(config as FlutterProjectConfig);
        }
...
function printConfigSummary(config: WebProjectConfig | FlutterProjectConfig): void {
  log.section('Project Summary');
  if (config.ecosystem === 'web') {
    const cfg = config as WebProjectConfig;
    printKeyValueTable([
      ['Project',    cfg.name],
      ['Ecosystem',  'Web'],
      ...
      ...(cfg.framework === 'next' ? [['src/ dir', cfg.srcDir ? '✓ Yes' : '✗ No'] as [string, string]] : []),
    ...
    const cfg = config as FlutterProjectConfig;
    printKeyValueTable([
      ['Project',      cfg.name],
      ['Ecosystem',    'Flutter'],
